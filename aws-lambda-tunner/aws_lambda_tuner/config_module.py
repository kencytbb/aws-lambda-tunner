...
class TunerConfig:
    ...
    def __post_init__(self):
        ...
        if self.workload_type not in ['on_demand', 'scheduled', 'continuous']:
            raise ConfigurationError("workload_type must be one of: on_demand, scheduled, continuous")
    ...
    def _validate_workload_configuration(self):
        ...
        if self.workload_type == "on_demand":
            if self.cold_start_sensitivity == "low" and self.memory_sizes and min(self.memory_sizes) < 512:
                warnings.warn(
    ...
    def validate_config(self, config: TunerConfig) -> List[str]:
        ...
        if config.workload_type == 'on_demand':
            if config.cold_start_sensitivity == 'high' and min(config.memory_sizes) < 512:
                warnings.append("On-demand workloads with high cold start sensitivity should use memory >= 512MB")
            if config.expected_concurrency > 50:
                warnings.append("On-demand workloads with high expected concurrency may benefit from provisioned concurrency")
        
        elif config.workload_type == 'continuous':
            if config.traffic_pattern == 'burst' and config.expected_concurrency > 100:
                warnings.append("Continuous workloads with burst patterns at high concurrency may experience throttling")
            if config.strategy == 'cost' and config.cold_start_sensitivity == 'high':
                warnings.append("Continuous workloads prioritizing cost may conflict with cold start requirements")
        
        elif config.workload_type == 'scheduled':
            if config.cold_start_sensitivity == 'high' and config.strategy == 'cost':
                warnings.append("Scheduled workloads with high cold start sensitivity may conflict with cost optimization")
            if config.traffic_pattern == 'burst' and config.iterations < 10:
                warnings.append("Scheduled workloads with burst patterns should use more iterations for reliable results")
        
        # Traffic pattern validations
        if config.traffic_pattern == 'burst' and config.concurrent_executions < 5:
            warnings.append("Burst traffic patterns should use higher concurrent executions for realistic testing")

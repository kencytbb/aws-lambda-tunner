...
class AlertManager:
    ...
    async def send_optimization_alert(self, optimization_event):
        ...
        alert = Alert(
            ...
            channels=[AlertChannel.EMAIL] if not optimization_event.auto_approve else [AlertChannel.CLOUDWATCH]
        )
    ...
    async def get_alert_statistics(self, time_window: timedelta = timedelta(days=7)) -> Dict[str, Any]:
        ...
        return {
            ...
            'most_common_alert_type': max(alerts_by_type.items(), key=lambda x: x[1])[0] if alerts_by_type else None
        }

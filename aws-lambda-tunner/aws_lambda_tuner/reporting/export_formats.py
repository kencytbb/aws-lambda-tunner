...
class PDFExporter:
    ...
    def __init__(self, results: Dict[str, Any]):
        ...
        if not REPORTLAB_AVAILABLE:
            raise ReportGenerationError("ReportLab is required for PDF export. Install with: pip install reportlab")
    ...
    def _create_title_page(self, workload_type: str) -> List:
        ...
        # Workload type
        workload_title = Paragraph(f"Workload Type: {workload_type.title()}", 
    ...
    def _create_executive_summary(self) -> List:
        ...
        if optimal and baseline:
            performance_gain = ((baseline['avg_duration'] - optimal['avg_duration']) / 
                              baseline['avg_duration'] * 100) if baseline['avg_duration'] > 0 else 0
            cost_savings = ((baseline['avg_cost'] - optimal['avg_cost']) / 
                           baseline['avg_cost'] * 100) if baseline['avg_cost'] > 0 else 0
    ...
    def _create_detailed_analysis(self) -> List:
        ...
        table_data = [['Memory (MB)', 'Avg Duration (ms)', 'Min (ms)', 'Max (ms)', 
                      'Cost per Invocation ($)', 'Success Rate (%)']]
        ...
        if len(table_data) > 1:
            config_table = Table(table_data, colWidths=[1*inch, 1.2*inch, 0.8*inch, 
                                                       0.8*inch, 1.5*inch, 1*inch])
    ...
    def _find_optimal_configuration(self) -> Optional[Dict[str, Any]]:
        ...
        for config in self.configurations:
            ...
            if successful:
                durations = [e['duration'] for e in successful]
                costs = [calculate_cost(config['memory_mb'], e.get('billed_duration', e['duration'])) 
                        for e in successful]
    ...
    def _get_baseline_stats(self) -> Optional[Dict[str, Any]]:
        ...
        if successful:
            durations = [e['duration'] for e in successful]
            costs = [calculate_cost(first_config['memory_mb'], e.get('billed_duration', e['duration'])) 
                    for e in successful]
...
class ExcelExporter:
    ...
    def __init__(self, results: Dict[str, Any]):
        ...
        if not EXCEL_AVAILABLE:
            raise ReportGenerationError("pandas and openpyxl are required for Excel export. Install with: pip install pandas openpyxl")
    ...
    def _create_summary_sheet(self, writer: pd.ExcelWriter, workload_type: str):
        ...
        summary_data = {
            ...
            'Value': [
                ...
                f"{((baseline['avg_duration'] - optimal['avg_duration']) / baseline['avg_duration'] * 100):.1f}" if optimal and baseline else 'N/A',
                f"{((baseline['avg_cost'] - optimal['avg_cost']) / baseline['avg_cost'] * 100):.1f}" if optimal and baseline else 'N/A',
        ...
        for config in self.configurations:
            ...
            for i, execution in enumerate(executions):
                raw_data.append({
                    ...
                    'Billed Duration (ms)': execution.get('billed_duration', execution.get('duration', 0)),
                    'Cost ($)': calculate_cost(memory_mb, execution.get('billed_duration', execution.get('duration', 0))),
...
class MultiFormatExporter:
    ...
    def export_report(self, output_path: str, format_type: str = "auto", 
                     workload_type: str = "generic", **kwargs):
        """
    ...
    def _export_json(self, output_path: str, **kwargs):
        ...
        export_data = {
            'metadata': {
                ...
                'total_executions': sum(len(c.get('executions', [])) 
        ...
        for config in self.results.get('configurations', []):
            ...
            for i, execution in enumerate(executions):
                csv_data.append({
                    ...
                    'billed_duration_ms': execution.get('billed_duration', execution.get('duration', 0)),
                    'cost_usd': calculate_cost(memory_mb, execution.get('billed_duration', execution.get('duration', 0))),
...
def export_multi_format(results: Dict[str, Any], output_path: str, format_type: str = "auto", 

...
from aws_lambda_tuner.reporting.service import ReportService
from aws_lambda_tuner.visualization_module import VisualizationGenerator
...
class TestCompleteOptimizationScenarios:
    ...
    def test_complete_cost_optimization_scenario(self, orchestrator, report_service, 
                                               visualization_generator, mock_aws_services, 
                                               validator, temp_dir):
    ...
    def test_complete_performance_optimization_scenario(self, orchestrator, report_service,
                                                       mock_aws_services, validator, temp_dir):
        ...
        improvement_ratio = (lowest_memory_duration - highest_memory_duration) / lowest_memory_duration
    ...
    def test_complete_balanced_optimization_scenario(self, orchestrator, mock_aws_services,
                                                   validator, temp_dir):
    ...
    def test_multi_function_optimization_scenario(self, orchestrator, mock_aws_services,
                                                validator, temp_dir):
    ...
    def test_error_recovery_scenario(self, orchestrator, mock_aws_services, validator):
        ...
        reliability_insights = [i for i in result.analysis.insights 
                              if i['type'] == 'reliability']
        assert len(reliability_insights) >= 1
        
        # Should still provide recommendations despite errors
        assert result.recommendation is not None
    
    def test_configuration_from_file_scenario(self, orchestrator, mock_aws_services, 
                                            validator, temp_dir):
    ...
    def test_continuous_optimization_scenario(self, orchestrator, mock_aws_services,
                                            validator, temp_dir):
        ...
        for day in range(3):  # Simulate 3 days of optimization
            ...
            config = TunerConfig(
                ...
                iterations=8,
                strategy='balanced',
                baseline_memory=optimization_history[-1]['optimal_memory'] if optimization_history else 256
            )
            ...
            optimization_record = {
                ...
                'optimal_memory': result.recommendation.optimal_memory_size,
                'should_optimize': result.recommendation.should_optimize,
                'avg_duration': result.memory_results[result.recommendation.optimal_memory_size].avg_duration,
                'avg_cost': result.memory_results[result.recommendation.optimal_memory_size].avg_cost,

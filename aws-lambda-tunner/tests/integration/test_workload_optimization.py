...
class TestWorkloadSpecificOptimization:
    ...
    def test_cpu_intensive_workload_optimization(self, orchestrator, mock_aws_services, validator):
        ...
        assert duration_improvement > 0.3, "CPU-intensive workload should show significant improvement with more memory"
    ...
    def test_io_bound_workload_optimization(self, orchestrator, mock_aws_services, validator):
        ...
        cold_start_insights = [i for i in result.analysis.insights 
                              if 'cold start' in i['message'].lower()]
        # May or may not be present depending on threshold
    
    @pytest.mark.workload_memory
    def test_memory_intensive_workload_optimization(self, orchestrator, mock_aws_services, validator):
        ...
        assert improvement_ratio > 0.15, "Memory-intensive workload should show significant memory sensitivity"
    ...
    def test_balanced_workload_optimization(self, orchestrator, mock_aws_services, validator):
        ...
        assert recommended_efficiency >= max_efficiency * 0.9, \
    ...
    def test_multi_iteration_workflow(self, orchestrator, mock_aws_services, validator):
        ...
        for memory_result in result.memory_results.values():
            ...
            if len(memory_result.raw_results) > 1:
                durations = [r['duration'] for r in memory_result.raw_results if r['status_code'] == 200]
                if durations:
...
class TestParametrizedWorkflows:
    ...
    @pytest.mark.parametrize("memory_range,expected_optimal_range", [
        ([128, 256, 512], (128, 512)),
        ([512, 1024, 2048], (512, 2048)),
        ([1024, 1536, 2048, 3008], (1024, 3008)),
    ])
    def test_different_memory_ranges(self, orchestrator, mock_aws_services, validator,
    ...
    def test_different_iteration_counts(self, orchestrator, mock_aws_services, validator, iteration_count):
    ...
    def test_different_concurrency_levels(self, orchestrator, mock_aws_services, validator, concurrent_count):

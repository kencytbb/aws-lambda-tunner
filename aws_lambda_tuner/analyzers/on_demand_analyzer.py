...
class OnDemandAnalyzer(PerformanceAnalyzer):
    ...
    def analyze_for_minimal_cold_starts(self, memory_results: Dict[int, MemoryTestResult]) -> Dict[str, Any]:
        ...
        optimal_config = self._find_cold_start_optimal_configuration(memory_results, cold_start_analysis)
        ...
        latency_impact = self._calculate_cold_start_latency_impact(memory_results, cold_start_analysis)
        ...
        return {
            ...
            "cold_start_metrics": {
                ...
                "avg_cold_start_penalty": cold_start_analysis.avg_cold_start_duration - cold_start_analysis.avg_warm_start_duration
            }
        }
    
    def get_on_demand_recommendation(self, memory_results: Dict[int, MemoryTestResult]) -> Recommendation:
        ...
        current_cold_start_ratio = current_result.cold_starts / current_result.iterations if current_result.iterations > 0 else 0
        optimal_cold_start_ratio = optimal_result.cold_starts / optimal_result.iterations if optimal_result.iterations > 0 else 0
        cold_start_improvement = ((current_cold_start_ratio - optimal_cold_start_ratio) / current_cold_start_ratio) * 100 if current_cold_start_ratio > 0 else 0
        ...
        reasoning = self._generate_on_demand_reasoning(
            ...
            cold_start_improvement, on_demand_analysis
        )
    ...
    def _find_cold_start_optimal_configuration(self, memory_results: Dict[int, MemoryTestResult], 
                                             cold_start_analysis: ColdStartAnalysis) -> Dict[str, Any]:
        ...
        for memory_size, result in memory_results.items():
            cold_start_ratio = result.cold_starts / result.iterations if result.iterations > 0 else 0
        ...
        return {
            ...
            "cold_start_ratio": optimal_result.cold_starts / optimal_result.iterations if optimal_result.iterations > 0 else 0,
    ...
    def _analyze_provisioned_concurrency_benefits(self, memory_results: Dict[int, MemoryTestResult], 
                                                cold_start_analysis: ColdStartAnalysis) -> Dict[str, Any]:
        ...
        return {
            ...
            "reason": f"Cold start ratio of {cold_start_analysis.cold_start_ratio:.2%} exceeds threshold of {self.cold_start_threshold:.2%}" if should_use_provisioned else "Cold start ratio is acceptable",
    ...
    def _calculate_cold_start_latency_impact(self, memory_results: Dict[int, MemoryTestResult], 
                                           cold_start_analysis: ColdStartAnalysis) -> Dict[str, Any]:
        ...
        cold_start_penalty = cold_start_analysis.avg_cold_start_duration - cold_start_analysis.avg_warm_start_duration
        ...
        return {
            ...
            "percentage_affected": (total_cold_starts / total_requests) * 100 if total_requests > 0 else 0,
            "average_penalty_ms": cold_start_penalty,
            "estimated_p95_impact_ms": p95_impact,
            "estimated_p99_impact_ms": p99_impact,
            "user_experience_rating": self._rate_user_experience_impact(cold_start_analysis.cold_start_ratio, cold_start_penalty)
        }
    
    def _rate_user_experience_impact(self, cold_start_ratio: float, cold_start_penalty: float) -> str:
    ...
    def _determine_on_demand_optimal_memory(self, memory_results: Dict[int, MemoryTestResult], 
                                          cold_start_analysis: ColdStartAnalysis,
                                          on_demand_analysis: Dict[str, Any]) -> int:
        ...
        if cold_start_ratio > self.cold_start_threshold:
            ...
            for memory_size, result in sorted_by_memory:
                test_cold_start_ratio = result.cold_starts / result.iterations if result.iterations > 0 else 0
    ...
    def _should_optimize_for_on_demand(self, current_memory: int, optimal_memory: int, 
                                     cost_change: float, duration_change: float, 
                                     cold_start_improvement: float) -> bool:
        """Determine if optimization is worthwhile for on-demand workloads."""
        ...
        significant_cold_start_improvement = cold_start_improvement > 20.0  # 20% reduction in cold starts
        acceptable_cost_increase = cost_change < 50.0  # Accept up to 50% cost increase for cold start reduction
        ...
        return significant_cold_start_improvement and acceptable_cost_increase and performance_not_degraded
    
    def _generate_on_demand_reasoning(self, current_memory: int, optimal_memory: int, 
                                    cost_change: float, duration_change: float, 
                                    cold_start_improvement: float, 
                                    on_demand_analysis: Dict[str, Any]) -> str:
    ...
    def _calculate_on_demand_confidence_score(self, analysis, optimal_memory: int, 
                                            cold_start_analysis: ColdStartAnalysis) -> float:
    ...
    def _estimate_on_demand_monthly_savings(self, current_result: MemoryTestResult, 
                                          optimal_result: MemoryTestResult,
                                          cold_start_improvement: float) -> Dict[str, Dict[str, float]]:

...
@click.option(
    "--template",
    "-t",
    type=click.Choice(["speed", "cost", "balanced", "comprehensive"]),
    default="balanced",
...
@click.option(
    "--iterations", "-i", type=int, default=10, help="Number of iterations per memory size"
)
@click.option(
    "--strategy",
    "-s",
    type=click.Choice(["speed", "cost", "balanced"]),
    default="balanced",
        ...
        if config:
            ...
            if not function_arn:
                raise ConfigurationError(
                    "Function ARN is required. Use --function-arn or provide a config file."
                )
...
@click.option(
    ...
    "-w",
    type=click.Choice(
        ["web_api", "batch_processing", "event_driven", "scheduled", "stream_processing"]
    ),
...
def workload_wizard(
    function_arn: str, workload_type: Optional[str], output_dir: str, interactive: bool
):
    ...
    try:
        ...
        tuner_config = _customize_config_for_workload(
            base_config, workload_type, workload_config, function_arn
        )
        ...
        dashboard.create_performance_dashboard(
            str(output_path / f"{workload_type}_dashboard.html"), workload_type
        )
...
@click.option(
    "--pattern",
    type=click.Choice(["steady", "bursty", "seasonal", "growth"]),
    default="steady",
    help="Traffic pattern for cost calculation",
)
@click.option("--output-dir", "-o", default="./cost-analysis", help="Output directory")
@click.option(
    "--format",
    "export_format",
    type=click.Choice(["html", "pdf", "excel", "json"]),
    default="html",
    help="Export format",
)
def cost_projection(
...
@click.option(
    "--interactive-dashboard", is_flag=True, help="Generate interactive comparison dashboard"
)
def compare_workloads(
    ...
    try:
        ...
        if len(workload_list) != len(results_files):
            click.echo(
                f"❌ Number of workload types ({len(workload_list)}) must match number of result files ({len(results_files)})"
            )
            sys.exit(1)
        ...
        if interactive_dashboard:
            ...
            create_workload_comparison_dashboard(
                comparison_data, str(output_path / "interactive_comparison.html")
            )
...
@click.option(
    ...
    "-w",
    type=click.Choice(
        ["web_api", "batch_processing", "event_driven", "scheduled", "stream_processing", "generic"]
    ),
    default="generic",
    help="Workload type for specialized dashboard",
)
...
def dashboard(
    ...
    output_dir: str,
    workload_type: str,
    include_cost_scenarios: bool,
    real_time_template: bool,
):
    ...
    try:
        ...
        dashboard_gen.create_performance_dashboard(
            str(output_path / "performance_dashboard.html"), workload_type
        )
        ...
        dashboard_gen.create_cost_analysis_dashboard(
            str(output_path / "cost_dashboard.html"), scenarios
        )
        click.echo("✅ Generated: cost_dashboard.html")

        # Cold start analysis dashboard
        dashboard_gen.create_cold_start_dashboard(str(output_path / "cold_start_dashboard.html"))
        click.echo("✅ Generated: cold_start_dashboard.html")

        # Comprehensive dashboard
        dashboard_gen.create_comprehensive_dashboard(
            str(output_path / "comprehensive_dashboard.html"), workload_type, scenarios
        )
        ...
        # Generate index page
        index_html = _generate_dashboard_index(
            workload_type, include_cost_scenarios, real_time_template
        )
...
@click.option(
    "--format",
    "export_format",
    type=click.Choice(["pdf", "excel", "json", "csv", "html"]),
    default="pdf",
    help="Export format",
)
@click.option(
    ...
    default="generic",
    help="Workload type for specialized export",
)
...
def _get_workload_config(workload_type: str, interactive: bool) -> Dict[str, Any]:
    ...
    if workload_type == "web_api":
        ...
        config["expected_concurrency"] = click.prompt(
            "Expected peak concurrency", default=10, type=int
        )
        config["cold_start_tolerance"] = click.prompt(
            "Cold start tolerance (%)", default=5, type=float
        )

    elif workload_type == "batch_processing":
        config["batch_size"] = click.prompt("Typical batch size", default=1000, type=int)
        config["processing_time"] = click.prompt(
            "Expected processing time (minutes)", default=10, type=int
        )
        ...
        config["reliability_target"] = click.prompt(
            "Target success rate (%)", default=99.9, type=float
        )
        ...
        config["time_window"] = click.prompt(
            "Acceptable execution window (minutes)", default=5, type=int
        )
        config["consistency_priority"] = click.confirm(
            "Prioritize execution consistency?", default=True
        )

    elif workload_type == "stream_processing":
        config["records_per_second"] = click.prompt("Records per second", default=1000, type=int)
        config["latency_requirement"] = click.prompt(
            "Max processing latency (ms)", default=500, type=int
        )
...
def _customize_config_for_workload(
    base_config: TunerConfig, workload_type: str, workload_config: Dict[str, Any], function_arn: str
) -> TunerConfig:
...
def _generate_cost_html_report(
    cost_analysis: Dict[str, Any], scenarios: List[Dict[str, Any]]
) -> str:
...
def _generate_comparison_html_report(
    comparison: Dict[str, Any], comparison_data: List[Dict[str, Any]]
) -> str:

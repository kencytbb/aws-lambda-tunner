...
class CostPredictor:
    ...
    def predict_costs_for_memory(
        ...
        try:
            ...
            base_costs = self._calculate_base_costs(memory_size, avg_duration, monthly_invocations)
            ...
            return CostPrediction(
                memory_size=memory_size,
                predicted_cost_per_invocation=(
                    basic_cost / monthly_invocations if monthly_invocations > 0 else 0
                ),
    ...
    def predict_costs_across_memories(
        ...
        for memory_size in memory_sizes:
            if memory_size in performance_data:
                ...
                prediction = self.predict_costs_for_memory(
                    memory_size, estimated_duration, monthly_invocations
                )
                predictions[memory_size] = prediction
    ...
    def find_cost_optimal_configuration(
        ...
        optimal_memory = min(
            valid_configs.keys(), key=lambda m: valid_configs[m].predicted_monthly_cost
        )

        return optimal_memory, valid_configs[optimal_memory]

    def analyze_cost_trends(self, cost_predictions: Dict[int, CostPrediction]) -> Dict[str, Any]:
        """
        ...
        return {
            "trend": "analyzed",
            "optimal_range": {"min_memory": memory_sizes[np.argmin(costs)], "min_cost": min(costs)},
            "sweet_spots": sweet_spots,
            "marginal_effectiveness": marginal_effectiveness,
            "cost_efficiency_trend": (
                "decreasing" if efficiency_derivatives[-1] < 0 else "increasing"
            ),
    ...
    def _calculate_base_costs(
        self, memory_size: int, avg_duration: float, monthly_invocations: int
    ) -> Dict[str, float]:
        ...
        return {
            ...
            "breakdown": {"compute": monthly_compute_cost, "requests": monthly_request_cost},
        }
    ...
    def _calculate_confidence_interval(
        self, predicted_cost: float, memory_size: int, avg_duration: float
    ) -> Tuple[float, float]:
    ...
    def _calculate_savings_potential(self, memory_size: int, predicted_cost: float) -> float:
        ...
        baseline_cost = (
            predicted_cost * (baseline_memory / memory_size) * 1.1
        )  # Assume 10% efficiency loss
    ...
    def _extract_workload_characteristics(self, result: MemoryTestResult) -> Dict[str, Any]:
        ...
        return {
            "cold_start_ratio": (
                result.cold_starts / total_executions if total_executions > 0 else 0.0
            ),
            "error_ratio": result.errors / total_executions if total_executions > 0 else 0.0,
            "duration_variance": (
                (result.p95_duration - result.avg_duration) / result.avg_duration
    ...
    def _estimate_duration_for_memory(
        self, memory_size: int, performance_data: Dict[int, MemoryTestResult]
    ) -> float:
    ...
    def _generate_cost_trend_recommendations(
        self, memory_sizes: List[int], costs: List[float], sweet_spots: List[int]
    ) -> List[str]:
        ...
        if sweet_spots:
            recommendations.append(
                f"Consider memory sizes around {sweet_spots[0]}MB for optimal cost efficiency"
            )
    ...
    def _store_prediction(
        self, prediction: CostPrediction, characteristics: Optional[Dict[str, Any]]
    ):
        ...
        try:
            ...
            predictions_file = (
                self.data_dir / f"cost_predictions_{datetime.now().strftime('%Y%m')}.jsonl"
            )

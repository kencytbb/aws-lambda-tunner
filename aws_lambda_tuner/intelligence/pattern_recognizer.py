...
class PatternRecognizer:
    ...
    def detect_usage_patterns(
        self, execution_data: List[Dict[str, Any]], time_window: timedelta = timedelta(days=30)
    ) -> UsagePattern:
        """
    ...
    def identify_optimization_opportunities(
        self, patterns: List[Pattern], current_config: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _analyze_memory_efficiency_patterns(
        ...
        for i in range(1, len(memory_sizes)):
            ...
            duration_improvement = (
                (prev_duration - curr_duration) / prev_duration if prev_duration > 0 else 0
            )
        ...
        if len(duration_improvements) >= 3:
            ...
            for i in range(1, len(duration_improvements)):
                if (
                    duration_improvements[i] < diminishing_threshold
                    and duration_improvements[i - 1] >= diminishing_threshold
                ):
    ...
    def _analyze_cold_start_patterns(
        ...
        high_cold_start_configs = [
            (memory, ratio)
            for memory, ratio in cold_start_ratios
    ...
    def _analyze_error_patterns(self, memory_results: Dict[int, MemoryTestResult]) -> List[Pattern]:
        """Analyze error patterns."""
        patterns = []
        ...
        high_error_configs = [
            (memory, rate) for memory, rate in error_rates if rate > high_error_threshold
        ]
        ...
        if len(memory_values) > 2 and max(rate_values) > 0:
            ...
            if abs(correlation) > 0.5:  # Strong correlation
                pattern_desc = (
                    "Higher memory reduces errors"
                    if correlation < 0
                    else "Higher memory increases errors"
                )
                patterns.append(
                    Pattern(
                        pattern_type=PatternType.ERROR,
                        ...
                        confidence=abs(correlation),
                        impact_score=0.8,
                        evidence={"correlation": correlation, "error_rates": error_rates},
                        recommendations=[
    ...
    def _analyze_temporal_patterns(self, historical_data: List[Dict[str, Any]]) -> List[Pattern]:
    ...
    def _calculate_variance_metrics(
        ...
        if daily_counts:
            ...
            metrics["daily_cv"] = (
                np.std(daily_values) / np.mean(daily_values) if np.mean(daily_values) > 0 else 0
            )

        if weekly_counts:
            weekly_values = list(weekly_counts.values())
            metrics["weekly_cv"] = (
                np.std(weekly_values) / np.mean(weekly_values) if np.mean(weekly_values) > 0 else 0
            )

        return metrics

    def _detect_seasonality(
        self, daily_counts: Dict[str, int], weekly_counts: Dict[str, int]
    ) -> Dict[str, Any]:
        """Detect seasonal patterns."""
        seasonality = {"daily_pattern": False, "weekly_pattern": False, "patterns": []}
                ...
                anomalies.append(
                    {
                        ...
                        "severity": (
                            (duration - threshold) / std_duration if std_duration > 0 else 0
                        ),
                    }
                )
    ...
    def _identify_memory_opportunities(
        self, performance_patterns: List[Pattern], current_config: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def _identify_cold_start_opportunities(
        self, cold_start_patterns: List[Pattern], current_config: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def _identify_cost_opportunities(
        self, cost_patterns: List[Pattern], current_config: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def _identify_error_opportunities(
        self, error_patterns: List[Pattern], current_config: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def _identify_temporal_opportunities(
        self, temporal_patterns: List[Pattern], current_config: Dict[str, Any]
    ) -> List[Dict[str, Any]]:

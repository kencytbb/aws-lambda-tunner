...
class PerformanceMonitor:
    ...
    async def get_current_metrics(
        self, time_window: timedelta = timedelta(minutes=15)
    ) -> Dict[str, float]:
        """
        ...
        try:
            ...
            for metric_name in self.monitored_metrics:
                metric_data = await self._get_cloudwatch_metric(metric_name, start_time, end_time)

                if metric_data:
                    if metric_name == "Duration":
                        metrics["avg_duration"] = (
                            np.mean([dp.value for dp in metric_data]) / 1000.0
                        )  # Convert to seconds
                        metrics["p95_duration"] = (
                            np.percentile([dp.value for dp in metric_data], 95) / 1000.0
                        )
                        metrics["p99_duration"] = (
                            np.percentile([dp.value for dp in metric_data], 99) / 1000.0
                        )
                    elif metric_name == "Errors":
                        error_count = sum(dp.value for dp in metric_data)
                        invocation_data = await self._get_cloudwatch_metric(
                            "Invocations", start_time, end_time
                        )
                        total_invocations = (
                            sum(dp.value for dp in invocation_data) if invocation_data else 1
                        )
                        metrics["error_rate"] = (
                            error_count / total_invocations if total_invocations > 0 else 0
                        )
                        ...
                        metrics["throttle_rate"] = (
                            throttle_count / total_invocations if total_invocations > 0 else 0
                        )
            ...
            metrics["cost_per_invocation"] = await self._calculate_current_cost_per_invocation(
                metrics
            )
    ...
    async def get_performance_trends(
        self, time_window: timedelta = timedelta(hours=24), metrics: Optional[List[str]] = None
    ...
    async def get_cost_metrics(
        self, time_window: timedelta = timedelta(days=1)
    ) -> Dict[str, float]:
        """
        ...
        try:
            ...
            return {
                ...
                "cost_per_invocation": (
                    total_cost / total_invocations if total_invocations > 0 else 0
                ),
    ...
    async def get_error_trends(self, time_window: timedelta = timedelta(hours=6)) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_error_rate = (
                np.mean(error_rates[-3:]) if len(error_rates) >= 3 else np.mean(error_rates)
            )
            historical_error_rate = (
                np.mean(error_rates[:-3]) if len(error_rates) > 3 else recent_error_rate
            )
    ...
    async def get_current_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_snapshot": (
                self.performance_snapshots[-1].timestamp.isoformat()
    ...
    async def get_recent_performance_data(
        ...
        try:
            ...
            memory_results = {
                current_metrics.get("memory_size", 1024): type(
                    "MemoryTestResult",
                    (),
                    {
                        ...
                        "total_cost": 0,
                        "cold_starts": int(
                            current_metrics.get("cold_start_rate", 0)
                            * current_metrics.get("invocation_count", 0)
                        ),
                        "errors": int(
    ...
    async def measure_configuration_impact(
        ...
        try:
            ...
            impact["memory_change"] = new_config.get("MemorySize", 0) - previous_config.get(
                "MemorySize", 0
            )
    ...
    async def _get_cloudwatch_metric(
        self, metric_name: str, start_time: datetime, end_time: datetime, period: int = 60
    ...
    async def _estimate_cold_start_rate(self, start_time: datetime, end_time: datetime) -> float:
    ...
    def _analyze_metric_trend(
        self, metric_name: str, data_points: List[MetricDataPoint]
    ) -> PerformanceTrend:
    ...
    async def _calculate_pattern_change_score(
        self, current_hourly: Dict[int, float], current_daily: Dict[int, float]
    ) -> float:
        ...
        hourly_cv = (
            np.std(hourly_values) / np.mean(hourly_values) if np.mean(hourly_values) > 0 else 0
        )

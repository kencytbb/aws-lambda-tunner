...
class ReportGenerator:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            performance_gain = (
                safe_divide(
                    baseline["avg_duration"] - optimal["avg_duration"], baseline["avg_duration"]
                )
                * 100
            )

            cost_savings = (
                safe_divide(baseline["avg_cost"] - optimal["avg_cost"], baseline["avg_cost"]) * 100
            )
...
"""
    ...
    def _calculate_config_statistics(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"avg_duration": statistics.mean(durations), "avg_cost": statistics.mean(costs)}
    ...
    def _process_configuration(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "statistics": {"duration": duration_stats, "cost": cost_stats},
    ...
    def _generate_recommendations(self) -> List[Dict[str, str]]:
        ...
        if baseline["avg_cost"] > optimal["avg_cost"]:
            monthly_savings = (
                (baseline["avg_cost"] - optimal["avg_cost"]) * 30 * 24 * 60 * 2
            )  # Assume 2 invocations per minute
            recommendations.append(
                {
                    "title": "Cost Savings",
        ...
        total_cold_starts = sum(
            c.get("cold_starts", 0) for c in self.results.get("configurations", [])
        )
    ...
    def comparative_analysis_reports(
        self, comparison_workloads: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for workload in comparison_workloads:
                ...
                temp_generator = ReportGenerator(workload_results, self.config, workload_type)
    ...
    def _web_api_analysis(
        self, optimal: Dict[str, Any], baseline: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _batch_processing_analysis(
        self, optimal: Dict[str, Any], baseline: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _stream_processing_analysis(
        self, optimal: Dict[str, Any], baseline: Dict[str, Any]
    ) -> Dict[str, Any]:
            ...
            "key_metrics": {
                ...
                * 100,
                "cost_savings": safe_divide(
                    baseline["avg_cost"] - optimal["avg_cost"], baseline["avg_cost"]
                )
                * 100,
            },
            "recommendations": [
    ...
    def _calculate_savings_analysis(self, projections: Dict[str, Any]) -> Dict[str, Any]:
        ...
        baseline_projections = self._get_baseline_cost_projection(
            [
                {
                    "name": name,
                    "daily_invocations": proj["invocations_per_day"],
                    "pattern": "steady",
        ...
        return {
            ...
            "average_savings_percentage": (
                statistics.mean([s["percentage_savings"] for s in scenario_savings.values()])
                if scenario_savings
                else 0
            ),
        }
            ...
            if executions:
                ...
                if successful:
                    avg_duration_seconds = (
                        statistics.mean([e["duration"] for e in successful]) / 1000
                    )
                    estimated_throughput = (
                        1 / avg_duration_seconds if avg_duration_seconds > 0 else 0
                    )
    ...
    def _get_event_driven_scaling_recommendations(
        self, optimal: Dict[str, Any]
    ) -> List[Dict[str, str]]:
    ...
    def _get_scheduled_scaling_recommendations(
        self, optimal: Dict[str, Any]
    ) -> List[Dict[str, str]]:
    ...
    def _generate_cross_workload_insights(
    ...
    def _generate_workload_best_practices(
        self, comparisons: List[Dict[str, Any]]
    ) -> List[Dict[str, str]]:

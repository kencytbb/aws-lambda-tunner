...
class InteractiveDashboard:
    ...
    def create_performance_dashboard(self, output_path: str, workload_type: str = "generic"):
        ...
        try:
            ...
            fig.add_trace(
                go.Bar(
                    x=[f"{m}MB" for m in memory_sizes],
                    y=avg_durations,
                    name="Avg Duration",
            ...
            for i, config in enumerate(self.configurations):
                ...
                fig.add_trace(
                    go.Box(y=durations, name=f"{config['memory_mb']}MB", boxpoints="outliers"),
                    row=2,
                    col=2,
                )
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    marker=dict(size=10, color=memory_sizes, colorscale="viridis", showscale=True),
                ),
                row=3,
                col=1,
            )
            ...
            table_data = [
                [
                    "Best Performance",
                    f"{memory_sizes[best_performance_idx]}MB",
                    f"{avg_durations[best_performance_idx]:.1f}ms",
                ],
                [
                    "Best Cost",
                    f"{memory_sizes[best_cost_idx]}MB",
                    f"${avg_costs[best_cost_idx]:.6f}",
                ],
                [
                    "Lowest Cold Start",
                    f"{memory_sizes[cold_start_rates.index(min(cold_start_rates))]}MB",
            ...
            fig.add_trace(
                go.Table(
                    header=dict(
                        values=["Metric", "Memory Size", "Value"],
                        fill_color="lightblue",
                        font=dict(size=12, color="white"),
                    ),
                    cells=dict(
                        values=list(zip(*table_data)), fill_color="lightgray", font=dict(size=11)
                    ),
                ),
                row=3,
                col=2,
            ...
            self._save_interactive_html(
                fig, output_path, f"{workload_type.title()} Performance Dashboard"
            )
    ...
    def create_cost_analysis_dashboard(
        self, output_path: str, scenarios: List[Dict[str, Any]] = None
    ):
        """
        ...
        try:
            ...
            fig.add_trace(
                go.Bar(
                    x=[f"{m}MB" for m in memory_sizes],
                    y=monthly_costs,
                    name="Monthly Cost (1M invocations)",
                    marker_color="orange",
        ...
        try:
            fig = make_subplots(
                ...
                subplot_titles=(
                    ...
                    "Cold vs Warm Duration",
                    "Cold Start Timeline",
                    "Memory Impact on Cold Starts",
                ),
                specs=[
                    [{"secondary_y": False}, {"secondary_y": False}],
                    [{"secondary_y": False}, {"secondary_y": False}],
            ...
            for memory in memory_sizes:
                ...
                if data["cold_durations"] and data["warm_durations"]:
                    ...
                    fig.add_trace(
                        go.Bar(
                            x=[f"{memory}MB Cold", f"{memory}MB Warm"],
                            y=[avg_cold, avg_warm],
                            name=f"{memory}MB",
            ...
            if all_executions:
                ...
                if cold_executions:
                    fig.add_trace(
                        go.Scatter(
                            x=[e["index"] for e in cold_executions],
                ...
                if warm_executions:
                    fig.add_trace(
                        go.Scatter(
                            x=[e["index"] for e in warm_executions],
            ...
            for memory in memory_sizes:
                data = memory_data[memory]
                cold_avg = (
                    sum(data["cold_durations"]) / len(data["cold_durations"])
                    if data["cold_durations"]
                    else 0
                )
                warm_avg = (
                    sum(data["warm_durations"]) / len(data["warm_durations"])
                    if data["warm_durations"]
                    else 0
                )
    ...
    def create_workload_comparison_dashboard(
        self, output_path: str, comparison_results: List[Dict[str, Any]]
    ):
        """
        ...
        try:
            ...
            fig.add_trace(
                go.Table(
                    ...
                    cells=dict(
                        values=list(zip(*table_data)), fill_color="lightgray", font=dict(size=10)
                    ),
                ),
                row=2,
                col=2,
...
def create_interactive_cost_dashboard(
...
def create_workload_comparison_dashboard(
    comparison_results: List[Dict[str, Any]], output_path: str
):

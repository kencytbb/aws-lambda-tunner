...
class ScheduledStrategy(WorkloadStrategy):
    ...
    def create_testing_strategy(self, workload: WorkloadCharacteristics) -> TestingStrategy:
        ...
        if (
            workload.invocation_frequency == "high"
            and workload.traffic_pattern == "bursty"
            and workload.peak_concurrency
            and workload.peak_concurrency > 10
        ):
    ...
    async def execute_optimization(
        self, workload: WorkloadCharacteristics, strategy: TestingStrategy
    ) -> OptimizationResult:
        """
    ...
    async def _test_schedule_simulation(
        self, memory_size: int, workload: Optional[WorkloadCharacteristics] = None
    ) -> Dict[str, Any]:
        """
        ...
        if immediate_results:
            simulation_results["immediate_start_performance"] = {
                "avg_duration": sum(r.get("duration", 0) for r in immediate_results)
                / len(immediate_results),
                "cold_start_ratio": sum(1 for r in immediate_results if r.get("cold_start", False))
                / len(immediate_results),
                "success_rate": sum(1 for r in immediate_results if not r.get("error"))
                / len(immediate_results)
                * 100,
            }
        ...
        if post_idle_results:
            simulation_results["post_idle_performance"] = {
                "avg_duration": sum(r.get("duration", 0) for r in post_idle_results)
                / len(post_idle_results),
                "cold_start_ratio": sum(1 for r in post_idle_results if r.get("cold_start", False))
                / len(post_idle_results),
                "success_rate": sum(1 for r in post_idle_results if not r.get("error"))
                / len(post_idle_results)
                * 100,
            }
        ...
        if immediate_duration > 0:
            consistency_score = (
                1 - abs(post_idle_duration - immediate_duration) / immediate_duration
            )
        ...
        simulation_results["schedule_readiness_score"] = (
    ...
    async def _test_burst_handling(self, memory_size: int) -> Dict[str, Any]:
        ...
        if successful_results:
            avg_duration = sum(r.get("duration", 0) for r in successful_results) / len(
                successful_results
            )
    ...
    async def _test_with_provisioned_concurrency(
        self, memory_size: int, pc_level: int, iterations: int, test_results: Dict[str, Any]
    ):
        """
    ...
    def _classify_scheduled_frequency(
        self, recent_metrics: Dict[str, Any], weekly_metrics: Dict[str, Any]
    ) -> str:
        """
    ...
    def _identify_execution_windows(
        self, weekly_metrics: Dict[str, Any]
    ...
    def _find_optimal_configuration(
        self, test_results: Dict[str, Any], workload: WorkloadCharacteristics
    ) -> Dict[str, Any]:
        """
        ...
        pc_text = (
            f" with {optimal['provisioned_concurrency']} PC"
            if optimal["provisioned_concurrency"]
            else ""
        )
        self.logger.info(
            f"Optimal configuration: {optimal['memory_size']}MB{pc_text} (score: {optimal['score']:.2f})"
        )

        return optimal

    def _calculate_scheduled_configuration_score(
        self, results: Dict[str, Any], workload: WorkloadCharacteristics
    ) -> float:
        """
        ...
        if "schedule_simulation" in results and "burst_handling" in results["schedule_simulation"]:
            burst_success_rate = results["schedule_simulation"]["burst_handling"].get(
    ...
    def _generate_recommendation(
        ...
        if baseline_results:
            ...
            optimal_cost = self._calculate_cost(
                optimal_memory, optimal_results.get("avg_billed_duration", 1000)
            )
            ...
            if baseline_duration > 0:
                performance_improvement_percent = (
                    (baseline_duration - optimal_duration) / baseline_duration
                ) * 100

            # Adjust confidence based on schedule readiness
            if "schedule_simulation" in optimal_results:
                readiness_score = optimal_results["schedule_simulation"].get(
                    "schedule_readiness_score", 0
                )
        ...
        if cost_savings_percent > 0:
            ...
            reasoning_parts.append(
                f"Cost increase of {abs(cost_savings_percent):.1f}% justified by reliability improvements"
            )

        if performance_improvement_percent > 0:
            reasoning_parts.append(
                f"Performance improvement of {performance_improvement_percent:.1f}% achieved"
            )
        ...
        if "resource_efficiency" in optimal_results:
            efficiency_rating = optimal_results["resource_efficiency"].get(
                "efficiency_rating", "unknown"
            )
        ...
        if not reasoning_parts:
            reasoning_parts.append(
                "Optimal configuration found based on scheduled workload analysis"
            )

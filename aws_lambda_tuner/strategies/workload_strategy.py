...
class WorkloadStrategy(ABC):
    ...
    async def optimize(self) -> OptimizationResult:
        ...
        self.logger.info(
            f"Created testing strategy with {len(testing_strategy.memory_sizes)} memory configurations"
        )

        # Execute optimization
        result = await self.execute_optimization(workload, testing_strategy)
        self.logger.info(
            f"Optimization completed with {result.confidence_score:.2f} confidence score"
        )
    ...
    def _determine_cold_start_sensitivity(
        self, workload_type: WorkloadType, metrics: Dict[str, Any]
    ) -> str:
        """
    ...
    async def _test_memory_configuration_with_monitoring(
        self, memory_size: int, iterations: int, monitor_duration_minutes: Optional[int] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _calculate_test_metrics(self, executions: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_billed_duration": (
                sum(billed_durations) / len(billed_durations) if billed_durations else 0
            ),
            "p95_duration": (

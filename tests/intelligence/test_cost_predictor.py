...
from aws_lambda_tuner.intelligence.cost_predictor import (
    CostPredictor,
    CostPrediction,
    WorkloadCostModel,
)
...
class TestCostPredictor:
    ...
    def test_savings_potential_calculation(self, sample_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            savings = predictor._calculate_savings_potential(
                memory_size=512, predicted_cost=80.0  # Lower than baseline
            )
    ...
    def test_duration_estimation(self, sample_config, sample_memory_results):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            estimated_duration = predictor._estimate_duration_for_memory(384, sample_memory_results)
            ...
            assert (
                min(duration_256, duration_512)
                <= estimated_duration
                <= max(duration_256, duration_512)
            )
    ...
    def test_basic_cost_calculation_fallback(self, sample_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            basic_cost = predictor._calculate_basic_cost(
                memory_size=512, avg_duration=1.5, monthly_invocations=10000
            )

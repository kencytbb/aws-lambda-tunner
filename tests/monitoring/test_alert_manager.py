...
from aws_lambda_tuner.monitoring.alert_manager import (
    AlertManager, Alert, AlertSeverity, AlertChannel, NotificationConfig
)
...
def notification_config():
    ...
    return NotificationConfig(
        ...
        sns_config={
            'topic_arn': 'arn:aws:sns:us-east-1:123456789012:lambda-alerts'
        }
    )
...
class TestAlertManager:
    ...
    async def test_get_alert_history(self, sample_config, notification_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            recent_history = await alert_manager.get_alert_history(
    ...
    async def test_get_alert_statistics(self, sample_config, notification_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            base_time = datetime.now()
            severities = [AlertSeverity.INFO, AlertSeverity.WARNING, AlertSeverity.ERROR, AlertSeverity.WARNING]
            types = ["performance_degradation", "cost_threshold", "error_rate_increase", "performance_degradation"]
    ...
    @pytest.mark.asyncio
    async def test_send_cloudwatch_alert(self, sample_config, notification_config, mock_cloudwatch_client):
    ...
    def test_message_formatting(self, sample_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            degradation_msg = alert_manager._format_degradation_message(
                "duration", 2.5, 1.5, 66.7
            )
            ...
            cost_msg = alert_manager._format_cost_threshold_message(
                150.0, 100.0, 50.0
            )
            ...
            error_msg = alert_manager._format_error_rate_message(
                0.12, 0.02, 120
            )

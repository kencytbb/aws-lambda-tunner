...
class TestPerformanceMonitor:
    ...
    async def test_get_cloudwatch_metric(self, sample_config, mock_cloudwatch_client):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            data_points = await monitor._get_cloudwatch_metric("Duration", start_time, end_time)
    ...
    async def test_get_cost_metrics(
        self, sample_config, mock_cloudwatch_client, mock_lambda_client
    ):
    ...
    async def test_analyze_traffic_patterns(self, sample_config, mock_cloudwatch_client):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert traffic_analysis["detected_pattern"] in [
                "burst",
                "steady",
                "variable",
                "unknown",
    ...
    async def test_measure_configuration_impact(self, sample_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            monitor.get_baseline_metrics = AsyncMock(
                return_value={
                    "avg_duration": 1.5,
                    "error_rate": 0.02,
            ...
            impact = await monitor.measure_configuration_impact(previous_config, new_config)

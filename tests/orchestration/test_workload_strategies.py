...
from aws_lambda_tuner.strategies.workload_strategy import (
    WorkloadStrategy, WorkloadType, WorkloadCharacteristics, TestingStrategy, OptimizationResult
)
...
class TestOnDemandStrategy(TestWorkloadStrategies):
    ...
    async def test_analyze_workload(self, on_demand_strategy):
        ...
        mock_concurrency = {
            'concurrent_executions': [
                {'value': 5}, {'value': 8}, {'value': 6}
            ]
        }
        
        with patch.object(on_demand_strategy, '_get_function_metrics') as mock_get_metrics, \
             patch.object(on_demand_strategy, '_get_concurrent_executions_metrics') as mock_get_concurrency:
    ...
    async def test_execute_optimization(self, on_demand_strategy, aws_provider):
        ...
        # Mock test results
        with patch.object(on_demand_strategy, '_test_memory_configuration_with_monitoring') as mock_test:
...
class TestContinuousStrategy(TestWorkloadStrategies):
    ...
    async def test_analyze_workload(self, continuous_strategy):
        ...
        mock_concurrency = {
            'concurrent_executions': [
                {'value': 20}, {'value': 25}, {'value': 18}
            ]
        }
        
        with patch.object(continuous_strategy, '_get_function_metrics') as mock_get_metrics, \
             patch.object(continuous_strategy, '_get_concurrent_executions_metrics') as mock_get_concurrency:
    ...
    def test_create_testing_strategy_high_frequency(self, continuous_strategy):
        ...
        assert strategy.provisioned_concurrency_levels is not None  # Should test PC for high frequency
    ...
    async def test_test_sustained_performance(self, continuous_strategy, aws_provider):
        ...
        aws_provider.invoke_function.side_effect = [
            # Wave 1
            {'duration': 500}, {'duration': 510}, {'duration': 505},
            {'duration': 520}, {'duration': 515}, {'duration': 525},
            {'duration': 530}, {'duration': 535}, {'duration': 528},
            {'duration': 540}, {'duration': 545}, {'duration': 538},
            # Wave 2
            {'duration': 515}, {'duration': 525}, {'duration': 520},
            {'duration': 535}, {'duration': 530}, {'duration': 540},
            {'duration': 545}, {'duration': 550}, {'duration': 542},
            {'duration': 555}, {'duration': 560}, {'duration': 548},
    ...
    def test_calculate_continuous_configuration_score(self, continuous_strategy):
        ...
        results = {
            ...
            'success_rate': 98,
            'executions': [
                {'duration': 500}, {'duration': 510}, {'duration': 490}
            ],
...
class TestScheduledStrategy(TestWorkloadStrategies):
    ...
    async def test_analyze_workload(self, scheduled_strategy):
        ...
        mock_weekly_metrics = {
            'invocations': [
                {'value': 50, 'timestamp': datetime.utcnow() - timedelta(days=i)}
                for i in range(7)
            ]
        }
        ...
        with patch.object(scheduled_strategy, '_get_function_metrics') as mock_get_metrics, \
             patch.object(scheduled_strategy, '_get_concurrent_executions_metrics') as mock_get_concurrency:
    ...
    def test_classify_scheduled_frequency(self, scheduled_strategy):
        ...
        weekly_metrics = {
            'invocations': [
                {'value': 50}, {'value': 60}, {'value': 40}, {'value': 55},
                {'value': 45}, {'value': 65}, {'value': 35}
            ]
        }
    ...
    def test_calculate_scheduled_configuration_score(self, scheduled_strategy):
        ...
        results = {
            ...
            'success_rate': 96,
            'executions': [
                {'duration': 600}, {'duration': 620}, {'duration': 580}
            ],
            'schedule_simulation': {
                'schedule_readiness_score': 0.9
            },
            'resource_efficiency': {
                'memory_utilization_ratio': 0.75
            },
            'idle_recovery': {
                'idle_impact_score': 0.2
            }
        }
...
class TestWorkloadStrategyBase(TestWorkloadStrategies):
    ...
    async def test_optimize_workflow(self, on_demand_strategy):
        ...
        with patch.object(on_demand_strategy, 'analyze_workload') as mock_analyze, \
             patch.object(on_demand_strategy, 'create_testing_strategy') as mock_create, \
             patch.object(on_demand_strategy, 'execute_optimization') as mock_execute:
    ...
    def test_analyze_traffic_pattern(self, on_demand_strategy):
        ...
        steady_metrics = {
            'invocations': [
                {'value': 100}, {'value': 105}, {'value': 95}, {'value': 102}
            ]
        }
        ...
        bursty_metrics = {
            'invocations': [
                {'value': 50}, {'value': 200}, {'value': 60}, {'value': 180}
            ]
        }
    ...
    @pytest.mark.asyncio
    async def test_test_memory_configuration_with_monitoring(self, on_demand_strategy, aws_provider):
        ...
        result = await on_demand_strategy._test_memory_configuration_with_monitoring(
            1024, 5
        )

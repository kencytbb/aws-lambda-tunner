...
def create_test_config(function_arn: str = None,
...
def create_test_results(function_arn: str = None,
                       memory_sizes: List[int] = None,
                       executions_per_memory: int = 5) -> Dict[str, Any]:
...
def create_test_recommendation(strategy: str = 'balanced',
                             current_memory: int = 256,
                             optimal_memory: int = 512) -> Recommendation:
    """Create a test recommendation."""
    cost_change = ((optimal_memory / current_memory) - 1) * 100
    duration_change = -((optimal_memory / current_memory) ** 0.5 - 1) * 100  # Negative = improvement
...
def create_complete_test_results(function_arn: str = None,
                               memory_sizes: List[int] = None) -> TuningResult:
    ...
    optimal_memory = min(memory_results.keys(), key=lambda x: 
                        memory_results[x].avg_duration * memory_results[x].avg_cost)
    recommendation = create_test_recommendation(
        current_memory=memory_sizes[0],

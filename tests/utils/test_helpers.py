...
class TestValidators:
    ...
    @staticmethod
    def validate_memory_test_result(
        result: MemoryTestResult, expected_memory_size: int = None, min_iterations: int = 1
    ) -> bool:
        """Validate MemoryTestResult structure and values."""
        assert isinstance(
            result, MemoryTestResult
        ), f"Expected MemoryTestResult, got {type(result)}"
        ...
        assert (
            result.p95_duration >= result.avg_duration
        ), "P95 duration must be >= average duration"
        ...
        assert (
            0 <= result.cold_starts <= result.iterations
        ), "Cold starts must be within iterations range"
        assert 0 <= result.errors <= result.iterations, "Errors must be within iterations range"

        if expected_memory_size:
            assert (
                result.memory_size == expected_memory_size
            ), f"Expected memory size {expected_memory_size}"
        ...
        if successful_iterations > 0 and result.avg_cost > 0:
            ...
            assert (
                abs(result.total_cost - expected_total) < 0.001
    ...
    def validate_recommendation(
        rec: Recommendation, current_memory: int = None, strategy: str = None
    ) -> bool:
        ...
        assert rec.strategy in [
            ...
            "balanced",
            "comprehensive",
        ], f"Invalid strategy: {rec.strategy}"
        ...
        if current_memory:
            assert (
                rec.current_memory_size == current_memory
            ), f"Expected current memory {current_memory}"
        ...
        if not rec.should_optimize:
            assert (
                rec.current_memory_size == rec.optimal_memory_size
        ...
        if rec.estimated_monthly_savings:
            for usage_level, savings_data in rec.estimated_monthly_savings.items():
                assert isinstance(
                    savings_data, dict
                ), f"Savings data must be dict for {usage_level}"
    ...
    def validate_performance_analysis(
        analysis: PerformanceAnalysis, expected_memory_sizes: List[int] = None
    ) -> bool:
        """Validate PerformanceAnalysis structure and values."""
        assert isinstance(
            analysis, PerformanceAnalysis
        ), f"Expected PerformanceAnalysis, got {type(analysis)}"
        ...
        for memory_size, score in analysis.efficiency_scores.items():
            assert (
                memory_size in analysis.memory_results
            ), f"Memory size {memory_size} not in results"
        ...
        for optimal_type, optimal_data in [
            ...
            if optimal_data:
                assert (
                    "memory_size" in optimal_data
                ), f"Missing memory_size in {optimal_type}_optimal"
                memory_size = optimal_data["memory_size"]
                assert (
                    memory_size in analysis.memory_results
                ), f"Optimal memory {memory_size} not in results"
        ...
        if expected_memory_sizes:
            ...
            ), f"Memory sizes mismatch: {actual_sizes} vs {expected_sizes}"

        return True

    @staticmethod
    def validate_tuning_result(result: TuningResult, expected_function_arn: str = None) -> bool:
        ...
        ), "Analysis must be PerformanceAnalysis"
        assert isinstance(
            result.recommendation, Recommendation
        ), "Recommendation must be Recommendation"
        assert result.duration >= 0, "Duration must be non-negative"

        # Validate ARN format
        arn_pattern = r"^arn:aws:lambda:[^:]+:\d+:function:[^:]+$"
        assert re.match(
            arn_pattern, result.function_arn
        ), f"Invalid ARN format: {result.function_arn}"

        if expected_function_arn:
            assert (
                result.function_arn == expected_function_arn
            ), f"Expected ARN {expected_function_arn}"
        ...
        assert (
            analysis_memory_sizes == result_memory_sizes
        ), "Memory sizes inconsistent between analysis and results"

        # Validate recommendation uses memory sizes from results
        assert (
            result.recommendation.current_memory_size in result_memory_sizes
            or result.recommendation.optimal_memory_size in result_memory_sizes
...
class TestAssertions:
    ...
    @staticmethod
    def assert_memory_performance_trend(
        memory_results: Dict[int, MemoryTestResult], expected_trend: str = "decreasing"
    ):
    ...
    @staticmethod
    def assert_cost_trend(
        memory_results: Dict[int, MemoryTestResult], expected_trend: str = "increasing"
    ):
        ...
        if expected_trend == "increasing":
            ...
            for i in range(1, len(costs)):
                assert costs[i] >= costs[i - 1] * 0.9, f"Cost should increase with memory: {costs}"
        elif expected_trend == "decreasing":
            # Cost decreases due to performance improvements outweighing memory cost
            assert costs[-1] <= costs[0] * 1.1, f"Expected decreasing cost trend: {costs}"

    @staticmethod
    def assert_efficiency_optimal(
        efficiency_scores: Dict[int, float], optimal_memory: int, tolerance: float = 0.1
    ):
    ...
    @staticmethod
    def assert_cold_start_impact(
        memory_results: Dict[int, MemoryTestResult], max_cold_start_rate: float = 0.5
    ):
        ...
        for memory_size, result in memory_results.items():
            ...
            assert (
                cold_start_rate <= max_cold_start_rate
            ), f"Cold start rate {cold_start_rate:.2%} too high for {memory_size}MB"

    @staticmethod
    def assert_error_rate_acceptable(
        memory_results: Dict[int, MemoryTestResult], max_error_rate: float = 0.1
    ):
    ...
    def assert_recommendation_consistency(
        recommendation: Recommendation, analysis: PerformanceAnalysis
    ):
        ...
        if recommendation.strategy == "cost":
            ...
            if cost_optimal_memory:
                assert (
                    recommendation.optimal_memory_size == cost_optimal_memory
            ...
            if speed_optimal_memory:
                assert (
                    recommendation.optimal_memory_size == speed_optimal_memory
            ...
            if balanced_optimal_memory:
                assert (
                    recommendation.optimal_memory_size == balanced_optimal_memory
...
class TestHelpers:
    ...
    def create_mock_response(
        ...
        if cold_start:
            log_lines.extend(
                ["INIT_START Runtime Version: python:3.9.v16", "INIT_REPORT Duration: 250.00 ms"]
            )
    ...
    def assert_approximately_equal(
        ...
        if expected == 0:
            assert abs(actual) <= tolerance, message or f"Expected ~{expected}, got {actual}"
        else:
            relative_error = abs(actual - expected) / abs(expected)
            assert relative_error <= tolerance, (
                message
                or f"Expected ~{expected}, got {actual} (relative error: {relative_error:.2%})"
            )

    @staticmethod
    def assert_timing_reasonable(
        start_time: datetime,
        end_time: datetime,
    ...
    @staticmethod
    def create_workload_specific_results(
        workload_type: str, memory_sizes: List[int] = None
    ) -> Dict[int, MemoryTestResult]:
        ...
        for memory_size in memory_sizes:
            results[memory_size] = generator.generate_memory_test_result(
                memory_size=memory_size, iterations=10, workload_type=workload_type
            )
...
class PerformanceTestHelpers:
    ...
    def create_load_test_data(
        num_functions: int = 10, executions_per_function: int = 100
    ) -> List[Dict[str, Any]]:
